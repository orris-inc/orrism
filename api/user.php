...
function mssm_user_create_account($params) {
    ...
    $data = [
        ...
        'uuid'          => mssm_uuid4(), // 使用我们的函数替代Uuid::uuid4()
        'token'         => mssm_generate_md5_token(),
    ...
    return mssm_new_account($data);
}

function mssm_user_suspend_account($params) {
    $sid = $params['serviceid'] ?? 0;
    $data = [
        'sid'    => $sid,
        'action' => 0,
    ];
    return mssm_set_status($data);
}

function mssm_user_unsuspend_account($params) {
    $sid = $params['serviceid'] ?? 0;
    $data = [
        'sid'    => $sid,
        'action' => 1,
...
function mssm_user_terminate_account($params) {
    $sid = $params['serviceid'] ?? 0;
    $data = [ 
        'sid' => $sid
    ];
    return mssm_delete_account($data);
}

function mssm_user_reset_uuid($params) {
    $sid = $params['serviceid'] ?? 0;
    $new_uuid = mssm_uuid4(); // Generate the new UUID
    $data = [
        'sid'  => $sid,
        'uuid' => $new_uuid
    ];
    
    $success = mssm_reset_uuid_internal($data); // This now returns true or false
    
    if ($success) {
        // Log success with the new UUID
        //error_log("MSSM_INFO: UUID and Token reset successful for service ID {$sid}. New UUID: {$new_uuid}");
        return [
            'status'   => 'success',
            'msg'      => MSSM_L::product_reset_uuid_success,
            'new_uuid' => $new_uuid // Optionally include the new UUID in the response
        ];
    } else {
        // Error details are already logged by mssm_reset_uuid_internal
        //error_log("MSSM_ERROR: UUID and Token reset failed for service ID {$sid}. Check previous logs for details.");
        return [
            'status' => 'error',
...
function mssm_user_admin_services_tab_fields($params) {
    try {
        $user = mssm_get_user($params['serviceid'] ?? 0)[0];
        $result = [
            'uuid'                => $user['uuid'],
            MSSM_L::admin_bandwidth    => mssm_convert_byte($user['bandwidth']),
            MSSM_L::common_upload      => mssm_convert_byte($user['u']),
            MSSM_L::common_download    => mssm_convert_byte($user['d']),
            MSSM_L::common_left        => mssm_convert_byte($user['bandwidth'] - ($user['u'] + $user['d'])),
            MSSM_L::common_used        => mssm_convert_byte($user['u'] + $user['d']),
            MSSM_L::common_created_at  => $user['created_at'],
        ];
        return $result;
    } catch (Exception $e) {
        return $e;
...
function mssm_user_admin_custom_button_array() {
    return [
        MSSM_L::product_reset_bandwidth => 'reset_bandwidth_admin',
        MSSM_L::product_reset_uuid      => 'module_reset_uuid',
    ];
}
 ...
 * @return array
 */
function mssm_user_client_area($params) {
    ...
    if (!class_exists('MSSM_L')) {
        return [
            ...
            'templateVariables' => [
                'error' => 'Class MSSM_L not found',
            ],
        ];
    }
    if (!defined('MSSM_L::admin_bandwidth')) {
        return [
            'tabOverviewReplacementTemplate' => 'error.tpl',
            'templateVariables' => [
                'error' => 'MSSM_L::admin_bandwidth not defined',
            ],
        ];
    }
            ...
            switch ($_GET['ssmAction']) {
                case 'ResetUUID':
                    if (isset($_GET['sid']) && $_GET['sid'] == ($params['serviceid'] ?? 0)) {
                        $reset_result = mssm_user_reset_uuid($params); // Get the structured result
                        ...
                        if ($reset_result['status'] === 'success') {
                            //error_log("MSSM_INFO: Client Area - UUID reset successful for service ID {$params['serviceid']}. Response: " . json_encode($reset_result));
                        } else {
                            //error_log("MSSM_ERROR: Client Area - UUID reset failed for service ID {$params['serviceid']}. Response: " . json_encode($reset_result));
                        }
                        exit;
                    } else {
                        header('Content-Type: application/json');
                ...
                case 'ResetBandwidth':
                    if (isset($_GET['sid']) && $_GET['sid'] == ($params['serviceid'] ?? 0)) {
                        // 直接输出JSON响应
                        mssm_traffic_reset_bandwidth_user($params);
                        header('Content-Type: application/json');
                        echo json_encode([
                            'status' => 'success',
                            'msg'    => MSSM_L::traffic_reset_success
                        ]);
                        exit;
                    } else {
                        header('Content-Type: application/json');
    ...
    try {
        ...
        if ($user) {
            ...
            $info = [
                'uuid'        => $uuid,
                'upload'      => mssm_convert_byte($user_traffic_upload),
                'download'    => mssm_convert_byte($user_traffic_download),
                'total_used'  => mssm_convert_byte($user_traffic_total),
                'left'        => mssm_convert_byte($left),
                'created_at'  => $created_at,
                'telegram_id' => $telegram_id,
                'bandwidth'   => mssm_convert_byte($bandwidth),
                'sid'         => $sid,
                'token'       => $token
            ];
            ...
            // 获取节点信息，添加错误处理
            try {
                $nodes = mssm_get_nodes($service_id);
            ...
            return [
                ...
                'templateVariables' => [
                    ...
                    'subscribe_url' => mssm_get_config()['subscribe_url'] ?? '',
            ...
            return [
                'tabOverviewReplacementTemplate' => 'error.tpl',
                'templateVariables' => [
                    'error' => MSSM_L::error_account_not_found,
                ],
            ];
        }
    } catch (Exception $e) {
 ...
 */
function mssm_new_account($data){
    try {
        if (empty(mssm_get_user($data['sid']))){
            $conn = mssm_get_db_connection();
            ...
            mssm_set_redis($data['sid'], $data['token'], 'set');
            mssm_set_redis("node_group_id_sid_{$data['sid']}", $data['node_group_id'], 'set');
            mssm_set_redis("uuid{$data['sid']}", $data['uuid'], 'set');
            return $action->execute();
        } else{
            return MSSM_L::error_account_already_exists;
        }
    } catch (Exception $e){
        return $e;
    }
 ...
 * @return bool|string|Exception
 */
function mssm_set_status($data){
    //error_log("MSSM_DEBUG: mssm_set_status called with data: " . print_r($data, true)); // Log input data
    try {
        $user_exists = count(mssm_get_user($data['sid'])) > 0;
        //error_log("MSSM_DEBUG: User exists for sid " . ($data['sid'] ?? 'N/A') . "? " . ($user_exists ? 'Yes' : 'No')); // Log user existence
        ...
        if ($user_exists){
            ...
            //error_log("MSSM_DEBUG: Database execute result for sid " . ($data['sid'] ?? 'N/A') . ": " . ($execute_result ? 'Success' : 'Failure')); // Log execution result
            return $execute_result;
        } else{
            //error_log("MSSM_DEBUG: User not found for sid " . ($data['sid'] ?? 'N/A') . ". Returning error_account_not_found."); // Log user not found
            return MSSM_L::error_account_not_found;
        }
    } catch (Exception $e){
        //error_log("MSSM_DEBUG: Exception in mssm_set_status for sid " . ($data['sid'] ?? 'N/A') . ": " . $e->getMessage()); // Log exception
        return $e;
    }
}
 ...
 * @return bool|string|Exception
 */
function mssm_reset_uuid_internal($data){
    try {
        // Call mssm_get_user once to check existence
        $user_info = mssm_get_user($data['sid']);
        if (count($user_info) > 0) {
            $conn = mssm_get_db_connection();
            $new_token = mssm_generate_md5_token(); // Generate a new token
            // $data['uuid'] is the new UUID passed from mssm_user_reset_uuid
            ...
            if ($db->execute()) {
                ...
                mssm_set_redis('uuid'.$data['sid'], null, 'del', 0);
                mssm_set_redis('uuid'.$data['sid'], $data['uuid'], 'set', 0);
                // Clear and set Redis cache for token (using sid as key for token)
                mssm_set_redis($data['sid'], null, 'del', 0); 
                mssm_set_redis($data['sid'], $new_token, 'set', 0);
                return true; // Successfully updated DB and Redis
            } else {
                //error_log("MSSM_ERROR: Database execute failed in mssm_reset_uuid_internal for SID: " . ($data['sid'] ?? 'N/A'));
                return false; // Database execution failed
            }
        } else {
            //error_log("MSSM_ERROR: User not found in mssm_reset_uuid_internal for SID: " . ($data['sid'] ?? 'N/A'));
            return false; // User not found
        }
    } catch (Exception $e) {
        //error_log("MSSM_ERROR: Exception in mssm_reset_uuid_internal for SID: " . ($data['sid'] ?? 'N/A') . " - " . $e->getMessage());
 ...
 * @return bool|string|Exception
 */
function mssm_set_bandwidth($data){
    try {
        if (count(mssm_get_user($data['sid'])) > 0) {
            $conn = mssm_get_db_connection();
 ...
 * @param int $sid
 */
function mssm_reset_user_traffic($sid) {
    if (mssm_get_user($sid) != null){
        $reset_traffic = array(
            'u' => 0,
            'd' => 0,
            'sid' => $sid
        );
        mssm_set_bandwidth($reset_traffic);
    }
}
 ...
 * @return string|null
 */
function mssm_get_uuid($sid){
    $redis_key = 'uuid'.$sid;
    $redis = mssm_get_redis_connection(0);
    $redis = mssm_set_redis($redis_key, null, 'get', 0);
    if ($redis) {
        return $redis;
    }
    $conn = mssm_get_db_connection();
