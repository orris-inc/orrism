...
// 获取API密钥
$api_token = mssm_get_config('api_key');
if (!$api_token) {
...
function verify_token($token) {
    $api_token = mssm_get_config('api_key') ?: '123456789';
...
function handleGetRequest($act, $node_id, $node_type) {
    ...
    switch ($act) {
        case 'user':
            // 获取指定节点的用户列表
            $users = mssm_get_node_users($node_id, $node_type);
            echo json_encode(['data' => $users]);
            break;
            
        case 'node_info':
            // 获取指定节点信息
            $node = mssm_get_node_info($node_id);
        ...
        case 'config':
            // 获取节点配置
            if ($node_id > 0) {
                $nodes = mssm_get_node_by_id($node_id);
            } else {
                $nodes = mssm_get_all_nodes();
            }
            $config = mssm_format_config($nodes);
...
function handlePostRequest($act, $node_id) {
    ...
    switch ($act) {
        case 'submit':
            // 处理流量上报
            $result = mssm_handle_traffic_report($data, $node_id);
            foreach ($data as $user_data) {
                mssm_report_traffic($user_data);
            }
        ...
        case 'sync_state_to_mysql':
            // 同步Redis中的状态到MySQL
            $result = mssm_sync_state_to_mysql();
            echo json_encode($result);
            break;
...
function handleNodeStatusReport($data, $node_id) {
    ...
    $redis = mssm_get_redis_connection();
    if (!$redis) {
        http_response_code(500);
 ...
 * 获取指定ID的节点
 * @param int $node_id 节点ID
 * @return array 节点数组（为了与mssm_get_all_nodes格式一致，返回数组）
 */
function mssm_get_node_by_id($node_id) {
    try {
        $conn = mssm_get_db_connection();
 ...
 * @return array 节点信息
 */
function mssm_get_node_info($node_id) {
    try {
        // 获取节点基本信息
        $conn = mssm_get_db_connection();
        $query = "SELECT * FROM `nodes` WHERE `id` = :node_id";
 ...
 * @return array 用户列表
 */
function mssm_get_node_users($node_id, $node_type) {
    try {
        ...
        foreach ($users as $user) {
            ...
            switch ($cipher) {
                case '2022-blake3-aes-128-gcm':
                    $password = mssm_uuidToBase64($user['uuid'], 16);
                    break;
                case '2022-blake3-aes-256-gcm':
                    $password = mssm_uuidToBase64($user['uuid'], 32);
                    break;
                default:
                    $password = null;
            }
 ...
 * @return array 处理结果
 */
function mssm_handle_traffic_report($data, $node_id) {
    if (!is_array($data)) {
 ...
 */
function mssm_format_config($nodes) {
    $config = [
        'inbounds' => []
    ];
        ...
        if (isset($node['node_method']) && !empty($node['node_method'])) {
            ...
            switch ($node['node_method']) {
                case '2022-blake3-aes-128-gcm':
                    ...
                    $inbound['settings']['password'] = mssm_get_server_key($ctime_ts, 16);
                    break;
                case '2022-blake3-aes-256-gcm':
                    $ctime_ts = is_numeric($node['ctime']) ? intval($node['ctime']) : strtotime($node['ctime']);
                    $inbound['settings']['password'] = mssm_get_server_key('1743405847', 32);
                    break;
            }
 ...
 * @return array 同步结果
 */
function mssm_sync_state_to_mysql() {
    $date = date('Ymd');
