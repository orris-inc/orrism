 ...
 */
function mssm_get_db_connection() {
 ...
 * @return Redis|null
 */
function mssm_get_redis_connection($index = 0) {
 ...
 * @return mixed 操作结果
 */
function mssm_set_redis($key, $value, $action, $index = 0, $ttl = 0) {
    try {
        $redis = mssm_get_redis_connection($index);
        if (!$redis) {
            return false;
        }
        switch ($action) {
 ...
 */
function mssm_check_and_init_tables() {
...
// 仅在必要时检查表结构
if (defined('MSSM_AUTO_CHECK_TABLES') && MSSM_AUTO_CHECK_TABLES) {
    try {
        mssm_check_and_init_tables();
    } catch (Exception $e) {
 ...
 * @return array 用户统计数据
 */
function mssm_get_user_redis_stats($sid) {
    $result = [
        'last_ip' => '',
        'last_access' => 0,
        'created_at' => 0,
    ...
    try {
        ...
        // 获取所有用户数据
        $user_data = mssm_set_redis($user_key, null, 'hGetAll', 0);
        if (!$user_data) {
            return $result;
        }
        ...
        // 获取过期时间
        $result['ttl'] = mssm_set_redis($user_key, null, 'ttl', 0);
...
function mssm_get_ip_stats($ip) {
    ...
    try {
        ...
        foreach ($result['used_by'] as $sid) {
            $user_stats = mssm_get_user_redis_stats($sid);
 ...
 * @return array 每日访问统计
 */
function mssm_get_daily_stats($days = 7) {
    $result = [];
    ...
    try {
        ...
        for ($i = 0; $i < $days; $i++) {
            ...
            $visits = mssm_set_redis($date_key, null, 'get', 0);
