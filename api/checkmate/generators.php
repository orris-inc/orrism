...
abstract class ConfigGenerator {
    ...
    protected function getNodePassword($node) {
        if (in_array($node['node_method'], ['2022-blake3-aes-128-gcm', '2022-blake3-aes-256-gcm'])) {
            ...
            $serverKey = orrism_get_server_key($ctime_ts, $len);
            $userKey = orrism_uuidToBase64($this->user['uuid'], $len);
...
class SurgeGenerator extends ConfigGenerator {
    public function generate() {
        ...
        $config_values = orrism_get_config(); 
        $subsDomain = $config_values['subscribe_url'] ?? '';
        if (empty($subsDomain)) {
            error_log("ORRISM API Services Error (surge_generate for SID: {$this->user['sid']}): subscribe_url not found in config.");
        }
