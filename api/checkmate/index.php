...
class SubscriptionService {
    ...
    private function getClientProducts() {
        $clientProductResult = mssm_get_client_products($this->sid);
    ...
    private function authenticateUser() {
        // 获取用户 UUID
        $userUuid = mssm_get_uuid($this->sid);
        if ($userUuid === null) {
            RequestHandler::handleError(
                '500 Internal Server Error', 
                "(mssm_get_uuid for SID: {$this->sid}): UUID not found for user.", 
                $this->clientIp, 
        ...
        // 获取用户 TOKEN
        $userToken = mssm_get_token($this->sid);
        if ($userToken === null) {
            RequestHandler::handleError(
                '500 Internal Server Error', 
                "(mssm_get_token for SID: {$this->sid}): Token not found for user.", 
                $this->clientIp, 
                'Error: Authentication failed. Please contact support.'
            );
        }
        ...
        // 记录成功的请求
        error_log("MSSM API Services Access [IP: {$this->clientIp}]: Successful request for SID: {$this->sid}, App: {$this->app}");
        
        // 记录IP到Redis
        Security::recordIpToRedis($this->sid, $this->clientIp, $this->app);
        
        // 获取用户数据
        $userDataArray = mssm_get_user($this->sid);
        if (empty($userDataArray) || !isset($userDataArray[0])) {
            RequestHandler::handleError(
                '500 Internal Server Error', 
                "(mssm_get_user for SID: {$this->sid}): User data not found.", 
                $this->clientIp, 
    ...
    private function getNodeData() {
        $data = mssm_get_nodes($this->sid);
        if (empty($data)) {
            RequestHandler::handleError(
                '404 Not Found', 
                "(mssm_get_nodes for SID: {$this->sid}): No nodes available.", 
                $this->clientIp, 
